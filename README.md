...
```
HTTP  →  Controller  →  Form  →  Service  →  Repository (interface)  →  DataMapper  →  PostgreSQL
                      (проверка   (оркестрация)  (транзакции,          (Yii2 DAO,
                       запроса)                   сборка агрегата)      SQL ↔ Entity)
```
...
- **`src/Schema/CarSchema.php`** — единственное описание ограничений колонок
  ...
  и пробега). Его используют и миграции при создании таблиц, и формы при
  проверке запроса, поэтому проверка и схема не могут разойтись. Полноту покрытия
...
По той же причине числовые поля уходят на сервер строками, как их ввёл
пользователь. Приведение на клиенте — это тоже правило, только молчаливое:
пока форма делала `parseInt`, привычная запись пробега «35 000» превращалась
в 35, объявление сохранялось с кодом 201, и об ошибке не узнавал никто.
...
| Файл | Что проверяет |
|------|---------------|
| `tests/unit/CarServiceTest.php` | создание объявления: три состояния `options`, нормализация цены, сбор всех ошибок сразу и три регрессии, найденные сравнением поведения до и после выделения форм — пустой `options`, имена полей в сообщениях, пустой `photo_url` |
