...
Валидация живёт на входной границе, в формах, а не в сервисе. Это
единственная из трёх границ приложения, где данным не доверяют: наружу в БД и
обратно идут уже проверенные сущности, поэтому мапперы ничего не
перепроверяют. Сервис после этого занимается только оркестрацией — 71 строка
вместо прежних 282.

Блок `options` разбирается отдельной моделью: Yii не валидирует вложенные
структуры, и её ошибки переносятся в родительскую форму с префиксом
`options.` — это контракт, на который завязан фронтенд.
...
В каталоге [`frontend/`](frontend/README.md) — витрина объявлений на **Next.js 16,
React 19 и TypeScript**: список с пагинацией, карточка объявления и форма
размещения.

Браузер к API напрямую не обращается: страницы собираются серверными
компонентами, форма отправляется через Server Action. Поэтому CORS не нужен,
адрес API не попадает в клиентский бандл, а в кластере запрос идёт к Service
внутри namespace, минуя ingress.

Правила валидации во фронтенде не дублируются — форма раскладывает карту
`поле → сообщение` из ответа `422` обратно по своим полям, включая вложенные
ключи вида `options.year`. Новое правило на бэкенде появляется в интерфейсе
само.
